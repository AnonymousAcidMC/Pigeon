...
public class EntityPigeon extends EntityMob implements IFakeEntity {
	...
	// Boolean telling if the pigeon is being rendered in a GUI and not in the
	// world.
	...
	public void onLivingUpdate() {
		...
		if (Block.isEqualTo(blockStateBelow.getBlock(), Blocks.water)
				|| Block.isEqualTo(blockStateBelow.getBlock(), Blocks.flowing_water)) {
			...
			flapWings = true;
		} else if (Block.isEqualTo(blockStateBelow.getBlock(), Blocks.lava)
				|| Block.isEqualTo(blockStateBelow.getBlock(), Blocks.flowing_lava)) {
				...
				if (soundTimer <= 0) {
					if (!(mc.currentScreen instanceof GuiIngameMenu)) {
						Utils.playClientSound(
								...
								rand.nextInt(2) + 0.5f, 1f, (float) posX, (float) posY, (float) posZ);
		...
		Collection<EntityItem> nearbyItems = world().getEntitiesWithinAABB(EntityItem.class,
				getEntityBoundingBox().expand(15, 15, 15));
		...
		if (peckItem) {
			if (!this.getEntityBoundingBox().intersectsWith(food.getEntityBoundingBox().expand(.4d, 0d, .4d))) {
				startPecking = false;
				setPosition(posX + (food.posX - posX) / 9, posY + (food.posY - posY) / 9,
						posZ + (food.posZ - posZ) / 9);
			} else {
				startPecking = true;
			}
		} else {
